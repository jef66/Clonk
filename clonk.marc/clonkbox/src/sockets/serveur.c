...
int init_socket(int type, unsigned short int port){
	int listenSocket;
	if (port < 1500 || port > 65000){
		fputs("Erreur : mauvais numero de port\n", stderr);
		return -1;
	}
	listenSocket = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
	if (listenSocket < 0) {
		fputs("Erreur : impossible de créer le socket en écoute\n", stderr);
		return -2;
	}
	puts("Success : socket crééé");
...
int main()
{
	...
	scanf("%hu", &listenPort);
	if( 0 >	( listenSocket = init_socket(42, listenPort)))	{
		exit(EXIT_FAILURE);
	}
	...
	if (bind(listenSocket,
		...
		fputs("Impossible de lier le socket en écoute\n", stderr);
		exit(EXIT_FAILURE);
	}
